(ns q9-oracle
  "The Clojure side of the whole-component acceptance: run `accela.main`
   ...
   * The i64 answers -- route count, as-int, as-bool, page-limit, store counts,
     ...
     `accela.main` and count. Nothing is rendered on the way, so a
   ...
   * The hash answers go through a rendering. `str-hash` is the same rolling
   ...
   accela is the first pilot whose entity table declares `:coerce {}` on ALL
   SIX specs, so the float arm the siblings are blocked on is never reached on
   any production path here. That removes this port's obvious control, and a
   harness with no control is a harness that cannot be read.

   The control kept in its place is `as-int` of \"12abc\", the cohort-wide
   reader-conditional divergence: the oracle's :cljs branch yields 12 and its
   :clj branch yields 0, and the artifact matches :clj. That row MUST differ
   when this harness is run under nbb. A run in which everything matches has
   not demonstrated it can detect a mismatch and should not be read as a pass."
  (:require [accela.main :as m]
            [clojure.string :as s]))
...
(def fixture-data
  ;; These mirror src/accela/whole_component.kotoba's own fixture-data exactly.
  {0 {:name "v" :jurisdiction "v"}
   1 {:name "v"}
   2 {:name "v" :jurisdiction "v" :bogus "x"}
   3 {:type "v" :number "v"}
   4 {:type "v" :number "v"}
   5 {}})
...
(defn seeded-store [n]
  (let [st (m/fresh-store)]
    (doseq [i (range n)] (m/persist! st "Project" (seeded-row i)))
    st))

;; accela carries no two-ref spec, so `expand`'s fold runs over a single pair
;; here. Its shape instead is a two-LEVEL chain, Inspection -> Permit ->
;; Project, where a ref's target is itself a referrer.
(defn cred-row [i]
  {:id (str "accela_per_" i)
   :projectId (str "accela_pro_" i)
   :type "building"
   :number (str "PN-" i)})

(defn seeded-both [n]
  (let [st (seeded-store n)] (m/persist! st "Permit" (cred-row 1)) st))

(defn row [label v] (println (str label "\t" v)))

;; --- i64 answers: no rendering between main.cljc and the number -----------

(println "; oracle = accela.main under nbb")
...
(println "; store")
(row "store 0" (count (m/query (seeded-store 3) "Project")))
(row "store 1" (let [st (seeded-store 3)] (m/retract! st "Project" "accela_pro_1")
                 (count (m/query st "Project"))))
(row "store 2" (let [st (seeded-store 3)] (m/retract! st "Project" "nope")
                 (count (m/query st "Project"))))
(row "store 3" (count (m/query (seeded-both 3) "Permit")))

(println "; filters")
(let [rows (m/query (seeded-store 5) "Project")
      fields (:fields (spec-for "Project"))]
  ...
  (row "filters 3" (count (m/apply-filters rows {:name "P-3"} fields)))
  (row "filters 4" (count (m/apply-filters rows {:name "P-99"} fields)))
...
(doseq [[n lim] [[5 2] [5 5] [5 0] [5 200] [3 2] [0 2]]]
  (let [rows (m/query (seeded-store n) "Project")
...
(let [mk (fn [] (seeded-store 2))]
  (row "handlers 0" (second (m/handle-create (mk) "Project" (fixture-data 0))))
  (row "handlers 1" (second (m/handle-create (mk) "Project" (fixture-data 1))))
  (row "handlers 2" (second (m/handle-create (mk) "Project" (fixture-data 2))))
  (row "handlers 3" (second (m/handle-list (mk) "Project" {})))
  (row "handlers 4" (second (m/handle-get (mk) "Project" "accela_pro_0" {})))
  (row "handlers 5" (second (m/handle-get (mk) "Project" "nope" {})))
  (row "handlers 6" (second (m/handle-delete (mk) "Project" "accela_pro_0")))
  (row "handlers 7" (second (m/handle-delete (mk) "Project" "nope"))))
...
(let [rec (cred-row 1)
      bom (:refs (spec-for "Permit"))
      part (:refs (spec-for "Project"))]
