(ns accela.main
  "Kotodama WASM entrypoint for the Accela clean-room actor (L4) — Clojure port.
  ...
  facts (`accela.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "accela")
(def tier "L4")
...
(defn as-int-kernel
  ...
  of accela.main/as-int stays here as the retained oracle."
  [v]
  (if (pos? v) v 0))
...
(defn emit-facts
  "EAVT facts for one record: {\"accela.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "accela-compat" :tier tier :entities entities} 200])
...
(defn start! [] :accela-compat/ready)
